...
function updateBoard() {
	...
	$.ajax({
		...
		success:function(res){
			...
			location.href='/question/get/' + ${questionNum} ;
		},
...
<main>
	<h3>게시글 수정</h3>
	<form id="updateForm" action="/question/update" method="post"
						onsubmit="return updateBoard();">
		<input type="hidden" name="questionNum" value="${q.questionNum}">
		<div>
		<label>제목</label><input type="text" id="questionTitle" name="questionTitle" value="${q.questionTitle}" >
		</div>
		<div>
		<label id="lbContents">내용</label>
			<textarea name="questionContents" rows="5">${q.questionContents}</textarea>
		</div>
		<div id="divBtn">
